...
- Sessione MCP stdio verificata end-to-end: `initialize` risponde, `tools/list` espone **34 tool** (incl. `parse_portfolio_export`, `get_portfolio_config`, `map_holdings_to_targets`, `save_portfolio_snapshot`, `rank_candidates`, `backtest_picker`, `resolve_isins`), `prompts/list` espone i **4 prompt** (`portfolio_review`, `stock_picker`, `rebalance`, `deploy_cash`); `tools/call map_holdings_to_targets` sulla fixture `tests/fixtures/broker_export_page_layout.csv` e `tools/call get_portfolio_config` rispondono correttamente.
