...
- `uv run pytest`: **1046 test verdi**, tutti offline (fixture sintetiche, provider mockati, backtest su prezzi seeded); coverage di riga 94,1% (`scripts/test_report.py` → `docs/TEST_REPORT.md`, una riga per test). Backtest reali e benchmark: `scripts/backtest_report.py` → `docs/BACKTEST.md`.
...
- Sessione MCP stdio verificata end-to-end: `initialize` risponde, `tools/list` espone **28 tool** (incl. `parse_portfolio_export`, `get_portfolio_config`, `map_holdings_to_targets`), `prompts/list` espone i **4 prompt** (`portfolio_review`, `stock_picker`, `rebalance`, `deploy_cash`); `tools/call map_holdings_to_targets` sulla fixture `tests/fixtures/broker_export_page_layout.csv` e `tools/call get_portfolio_config` rispondono correttamente.
...
- Tool MCP: `parse_portfolio_export`, `get_portfolio_config`, `portfolio_risk`, `portfolio_exposure`, `map_holdings_to_targets`, `analyze_stock`, `screen_stocks`, `discover_stocks`, `company_facts`, `filing_sections`, `insider_activity`, `investor_relations_links`, `fx_rates`, `convert_amount_to_eur`, `macro_snapshot`, `allocate_cash`, `rebalance_portfolio`, `generate_order_plan`, `capital_auction`, `build_investment_plan`, `backtest_plan`, `save_thesis`, `check_thesis`, `propose_replacement`, `log_decision`, `review_decisions`, `personal_edge`, `decision_quality`. Prompt: `portfolio_review`, `stock_picker`, `rebalance`, `deploy_cash`.
